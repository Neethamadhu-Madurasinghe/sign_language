...
def read_all_files(path):
    ...
    for root, _, files in os.walk(path):  # Walk through all directories and subdirectories
...
def process_video(video_path):
    ...
    relative_path = os.path.relpath(video_path, input_path)

    # Split path into parts
    parts = relative_path.split(os.sep)

    # Process the class name (first directory)
    if len(parts) > 1:
        parts[1] = re.sub(r"^\d+\.\s*", "", parts[1])  # Remove leading number, dot, and space

    # Reconstruct the relative path with cleaned class name
    new_relative_path = os.path.join(*parts)

    # Replace file extension with .pkl
    new_relative_path = os.path.splitext(new_relative_path)[0] + ".pkl"

    # Construct the final output path
    output_path = os.path.join(output_dir, new_relative_path)


    print("Path file will be saved: " + output_path)
    os.makedirs(os.path.dirname(output_path), exist_ok=True)


    # Capture the video

    print("Video taken from: " + video_path)
